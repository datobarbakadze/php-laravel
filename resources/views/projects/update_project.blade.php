        ...
        <div class="container">
            <div class="row">
                @if(request()->user()!=null && request()->user()->role==1)
                    <form action="{{ route("updateProject",['id'=>$itemData->id]) }}" enctype="multipart/form-data" method="POST"  class="form-group col-sm-12 admin-form" >
                        ...
                        <select class="mt-3 mb-2form-control p-4 store-select"  name="status" id="store-select">
                            <option {{ $itemData->status=="current" ? 'selected' : '' }} value="current">ამჟამინდელი</option>
                            <option {{ $itemData->status=="finished" ? 'selected' : '' }} value="finished">დასრულებული</option>
