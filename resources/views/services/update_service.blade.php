        ...
        <div class="container">
            <div class="row">
                @if(request()->user()!=null && request()->user()->role==1)
                    <form action="{{ route("updateService",['id'=>$itemData->id]) }}" enctype="multipart/form-data" method="POST"  class="form-group col-sm-12 admin-form" >
                        ...
                        <select class="mt-3 mb-2form-control p-4 store-select"  name="type" id="store-select">
                            <option {{ $itemData->type=="training" ? 'selected' : '' }} value="training">ტრენინგები</option>
                            <option {{ $itemData->type=="service" ? 'selected' : '' }} value="service">ჩვენი სერვისები</option>
                            <option {{ $itemData->type=="farmer" ? 'selected' : '' }} value="farmer">ფერმერთა მომსახურება</option>
                        </select><br>
